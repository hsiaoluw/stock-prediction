...
class DataGeneratorSeq(object):
	...
	def _get_onestep_weekbatch(self, i, test=False):
		...
		for b in range(self._batch_size):
			week_batch_data[b] = self._week_data[int(self._day_week_map[int(cursor[b])]-i)] if not (self._output_norm_dict[0,0]==1)else self.five_day_avg_norm[int(cursor[b])-(i-1)*5]
			#week_batch_data[b] = self.five_day_avg_norm[int(cursor[b])-(i-1)*5]
